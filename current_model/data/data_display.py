...
def draw_graph(x_label,y_label,data,graph_texture):
    ...
    pr.draw_line(line_pos_x, line_pos_y - 200, line_pos_x + line_width, line_pos_y - 200,pr.BLACK) # x axis
    pr.draw_line(line_pos_x, line_pos_y, line_pos_x, line_pos_y - line_height, pr.BLACK ) # y axis

    pr.draw_text(x_label, screen_pos_x +(width//2) - (len(x_label) * 6) ,screen_pos_y + height - 25 - 200, 20,pr.BLACK)
    ...
    if data.data[len(data.data)-1][1] == 0:
        ...
        unit_y = (line_height - 200) / data.data[len(data.data)-1][1]

    for i in range(1,len(data.data)):
        pr.draw_line(line_pos_x + int(data.data[i][0]*unit_x) ,line_pos_y - int(data.data[i][1]*unit_y) - 200, line_pos_x + int(data.data[i-1][0]*unit_x) ,line_pos_y - int(data.data[i-1][1]*unit_y) -200 ,pr.BLUE)
